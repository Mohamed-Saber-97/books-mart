...
@Table(
        name = "buyer",
        indexes = {@Index(name = "idx_buyer_category", columnList = "category_id"), @Index(name = "idx_buyer_product", columnList = "product_id")}
)
@Getter
@NoArgsConstructor
@AllArgsConstructor
...
public class Buyer extends BaseEntity< Long > {
    ...
    public void removeFromCart(Product product) {
        if ( this.cart != null ) {
            this.cart.entrySet()
                     .removeIf(entry -> Objects.equals(entry.getKey()
                                                            .getId(), product.getId()));
        }
    }
