...
public interface ProductRepository extends JpaRepository< Product, Long > {
    ...
    @Query("SELECT p FROM Product p WHERE p.isDeleted = false AND p.quantity > 0") public List< Product > findAllAvailable();
    
    public Product findByIdAndIsDeletedIsFalse(Long id);
    
    public boolean existsByIsbn(String isbn);
    
    @Query("SELECT product FROM Product product where product.id in :productIds AND product.isDeleted = false AND product.quantity > 0")
